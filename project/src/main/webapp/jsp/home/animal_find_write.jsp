...
$(document).ready(function() {
  ...
  var domEleArray2 = [$('#attachFile2').clone()]; // 원본 복제
  $('#btn_delAttach2').click(function() {
    domEleArray2[1] = domEleArray2[0].clone(true); // 쌔거(0번) -> 복제본(1번)
    $('#attachFile2').replaceWith(domEleArray2[1]);
  });
  
  var domEleArray3 = [$('#attachFile3').clone()]; // 원본 복제
  $('#btn_delAttach3').click(function() {
    domEleArray3[1] = domEleArray3[0].clone(true); // 쌔거(0번) -> 복제본(1번)
    $('#attachFile3').replaceWith(domEleArray3[1]);
  });
});
</script>
...
<div style="margin: 0 auto; margin-top:100px;">
	<form action="animal_find_writeProc" method='post' id="f" enctype="multipart/form-data">
	<div class="card">
			...
			<div class="msg">
				...
				<input type="file" name="upfile" id="attachFile">
				<button type="button" id="btn_delAttach">x</button>
				<span style="color: lightcoral">해당 사진이 썸네일로 등록됩니다.</span><br>
				<input type="file" name="upfile2" id="attachFile2">
				<button type="button" id="btn_delAttach2">x</button><br>
				<input type="file" name="upfile3" id="attachFile3">
