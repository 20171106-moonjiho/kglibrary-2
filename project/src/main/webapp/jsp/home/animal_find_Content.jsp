...
<div align="center" style="margin: 0 auto; margin-top:100px;">
	...
	<c:choose>
		<c:when test="${board.image!=null}">
			<div class="swiper-slide"><img src="img/${board.image}"></div>
		</c:when>
		<c:when test="${board.image2!=null}">
			<div class="swiper-slide"><img src="img/${board.image2}"></div>
		</c:when>
		<c:when test="${board.image3!=null}">
			<div class="swiper-slide"><img src="img/${board.image3}"></div>
		</c:when>
	<c:otherwise>
			<div class="swiper-slide"><img src="../img/no_image.gif"></div>
	</c:otherwise>
	...
	<tr>
		<td colspan="4">
			<button type="button" onclick="location.href='animal_find'">목록</button>
			<c:if test="${sessionScope.id eq board.id}">
			<button type="button" onclick="location.href='animal_find_Modify?no=${board.no }'">수정</button>
