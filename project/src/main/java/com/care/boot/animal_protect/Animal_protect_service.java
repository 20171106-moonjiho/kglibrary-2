...
@Service
public class Animal_protect_service {

	@Autowired
	private Animal_protect_Mapper mapper;
	@Autowired
	...
	public void animal_protect(String animal_group, String search, String area, String cp, Model model) {
		...
		ArrayList<Animal_protect_DTO> boards = mapper.animal_protect(begin, end, animal_group, search, area);
		
		if(boards != null) {
		for (Animal_protect_DTO b : boards) {
			if (b.getImage() !=null){
		...
		String url = "animal_protect?animal_group=" + animal_group + "&search=" + search + "&area=" + area + "&currentPage=";
	...
	public String animal_protect_writeProc(MultipartHttpServletRequest multi) {
		...
		Animal_protect_DTO board = new Animal_protect_DTO();
		...
		mapper.animal_protect_writeProc(board);
		return "redirect:animal_protect";
	}


	public Animal_protect_DTO animal_protect_Content(String no) {
		int n = 1;
		try {
		...
		Animal_protect_DTO board = mapper.animal_protect_Content(n);
	...
	public String animal_protect_Modify(String no, Model model) {
		...
		return "animal_protect/animal_protect_Modify";
		
	}
	
	//수정 프로
	public String animal_protect_ModifyProc(Animal_protect_DTO board, MultipartHttpServletRequest multi) {
		Animal_protect_DTO check = mapper.animal_protect_Content(board.getNo());
		if(check == null)
			return "게시글 번호에 문제가 발생했습니다. 다시 시도하세요.";
		String sessionId = (String)session.getAttribute("id");	
		...
		int result = mapper.animal_protect_ModifyProc(board);
		if(result == 0)
	...
	public void animal_protect_main(Model model) {
		
		ArrayList<Animal_protect_DTO> boards2 = mapper.animal_protect_main();
		
		if(boards2 != null) {
		for (Animal_protect_DTO b : boards2) {
			if (b.getImage() !=null){
					String[] names= b.getImage().split("/");
					for(String name : names) {
					System.out.println("name: " +name);
