...
public class ManagementBoardService {
	...
	@Autowired IManagementBoardMapper mapper;
	@Autowired HttpSession session;
	
	public void managementForm(String cp, Model model, String search, String select) {
		int currentPage = 1;
		try{
		...
		List<ManagementBoardDTO> boards = mapper.managementForm(begin, end, select, search);
		int totalCount = mapper.totalCount(select, search);
		if(totalCount == 0) {
			return ;
		}
		
		String url = "managementForm?currentPage=";
	...
	public String managementWriteProc(MultipartHttpServletRequest multi) {
		...
		if(title == null || title.trim().isEmpty()) {
			return "redirect:managementBoardWrite";
		}
		
		ManagementBoardDTO board = new ManagementBoardDTO();
		...
		mapper.managementWriteProc(board);
		return "redirect:managementForm";
	}

	public ManagementBoardDTO managementBoardContent(String no) {
		int n = 1;
		try{
		...
		ManagementBoardDTO board = mapper.managementBoardContent(n);
	...
	public String managementBoardDeleteProc(String no) {
		...
		mapper.managementBoardDeleteProc(n);
		return "게시글 삭제 완료";
	}
	...
	public String managementBoardModifyProc(MultipartHttpServletRequest multi, String no) {
		...
		String title = multi.getParameter("title");
		if(title == null || title.trim().isEmpty()) {
			return "redirect:managementBoardModify";
		}
		...
		mapper.managementBoardModify(board);
		return "redirect:managementForm";
		
	}

	public ManagementBoardDTO managementBoardModify(String no) {
		int n = 1;
		try{
			n = Integer.parseInt(no);
		}catch(Exception e){
